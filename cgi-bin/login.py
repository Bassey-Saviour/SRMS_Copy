...
from utils import html_header, html_footer, create_session_cookie_value
...
def main():
  ...
  try:
    ...
    if bcrypt.checkpw(password_bytes, hashed_pw_bytes):
      ...
      # Redirect to the static student portal which will fetch profile via the session
      target = f"/public/student_portal.html"
      print("Status: 303 See Other")
      print(f"Location: {target}")
      # set cookie before printing the blank line that ends headers
      print(f"Set-Cookie: session={cookie_val}; HttpOnly; Path=/; Max-Age={max_age}")
